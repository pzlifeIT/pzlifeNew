...
class Coupons extends AdminController {
     ...
     * @apiSuccess (返回) {String} code 200:成功 / 3001:活动id有误 / 3002:image有误 / 3003:kind有误 / 3004:relevance有误 / 3005:debris有误 / 3006:title有误 / 3007:probability有误 / 3008:奖品最大设置个数为8 / 3009:总抽奖概率大于1 / 3010:有效的通用碎片已添加  / 3012:该商品SKU不存在 / 3013:该优惠券不存在
     * @apiSuccess (返回) {String} msg 返回消息
    ...
    public function saveHdGoods() {
        ...
        if (!empty($order)) {
            $result = $this->app->coupons->saveHdGoods($id, '', 0, 0, 0, '', 0, 0, 0, $order);
        } else {

            $result = $this->app->coupons->loseHdGoods($id);
        }
        $this->apiLog($apiName, [$cmsConId, $id], $result['code'], $cmsConId);
        return $result;
    }
