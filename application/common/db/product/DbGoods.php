...
use app\common\model\GrouponGoods;
...
//            array_push($result, $val);
    ...
    public function getGrouponGoods($where, $field, $row = false, $orderBy = '', $limit = '')
    {
        $obj = GrouponGoods::field($field)->where($where);
        return getResult($obj, $row, $orderBy, $limit);
    }

    public function countGrouponGoods($where)
    {
        return GrouponGoods::where($where)->count();
    }

    public function addGrouponGoods($data)
    {
        $Sheet = new GrouponGoods;
        $Sheet->save($data);
        return $Sheet->id;
    }

    public function updateGrouponGoods($data, $id)
    {
        $Sheet = new GrouponGoods;
        return $Sheet->save($data, ['id' => $id]);
    }
