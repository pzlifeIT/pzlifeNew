...
class Cart extends MyController {
    ...
    public function getUserCartNum() {
        ...
        /**
     * @api              {post} / 获取积分购物车商品
     * @apiDescription   getUserIntegralCart
     * @apiGroup         index_Cart
     * @apiName          getUserIntegralCart
     * @apiParam (入参) {Number} con_id
     * @apiSuccess (返回) {String} code 200:成功 / 3000:未获取到数据 / 3001.参数必须是数字 / 3002.参数不存在
     * @apiSuccess (返回) {Number} total 总条数
     * @apiSuccess (返回) {String} type_name 上级分类的name
     ...
     * @apiSuccess (failure) {String} goods_sku_name 规格
     * @apiSuccess (failure) {String} image 商品标题图
     * @apiSampleRequest /index/cart/getUserIntegralCart
     * @author rzc
     */
    public function getUserIntegralCart() {
        $apiName = classBasename($this) . '/' . __function__;
        $conId   = trim($this->request->post('con_id'));
        if (empty($conId)) {
            return ['code' => '3002'];
        ...
        // RVYvaEw2Wk1TeXlnUjdlb2RHc3ZEZz09
        // $uid = $this->app->user->deUid($paramUid);
        $result = $this->app->cart->getUserIntegralCart($conId);
        $this->apiLog($apiName, [$conId], $result['code'], $conId);
        return $result;
    }

    /**
     * @api              {post} / 添加积分商城购物车商品
     * @apiDescription   addUserIntegralCart
     * @apiGroup         index_Cart
     * @apiName          addUserIntegralCart
     * @apiParam (入参) {Number} con_id 请求uid
     * @apiParam (入参) {Number} goods_skuid 商品SKU_id
     * @apiParam (入参) {Number} goods_num 数量
     * @apiSuccess (返回) {String} code 200:成功 / 3000:未获取到数据 / 3001:uid长度只能是32位 / 3002:缺少参数conId / 3003:商品SKU_ID必须是数字 / 3004:购买数量必须是数字 / 3005:该商品钻石会员及以上身份专享 / 3006:库存不足购买数量 / 3007:该商品创业店主及以上身份专享 / 3008:该商品合伙人及以上身份专享 / 3009:缺少参数:uid或者商品SKUID / 3010:库存不足
     ...
     * @apiSuccess (data) {String} subtitle 副标题
     * @apiSuccess (data) {String} image 商品标题图
     * @apiSampleRequest /index/cart/addUserIntegralCart
     * @author rzc
     */
    public function addUserIntegralCart(){
        $apiName     = classBasename($this) . '/' . __function__;
        $conId       = trim($this->request->post('con_id'));
        $goods_skuid = trim($this->request->post('goods_skuid'));
        $goods_num   = trim($this->request->post('goods_num'));
